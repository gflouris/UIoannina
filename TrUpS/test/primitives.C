...
int primitives(){
   ...
   ///EventTree    
    ...
    TH2F *phib_int_out_2d = new TH2F("phib_int_out_2d","phib_int_out_2d",2048,-2048,2048, 2048,-2048,2048);
  ...
  TH2I* h_nin_nout  = new TH2I("h_nin_nout","h_nin_nout",50,0,50, 50, 0,50);
   ...
   for(int i=0; i<nentries; i++){
    //for(int i=0; i<50; i++){
       ...
       h_nin_nout->Fill(l1TC.phSize, l1TC.tmSize);
     ...
     for(int tp=0; tp<l1TC.tmSize; tp++){ 
      ...
      if(l1TC.tmRpcBit[tp]==1)
        h2_bx->Fill(l1TC.tmBx[tp]);

      if(l1TC.tmRpcBit[tp]==0)
        h3_bx->Fill(l1TC.tmBx[tp]);
      if(l1TC.tmRpcBit[tp]==2)
        h4_bx->Fill(l1TC.tmBx[tp]);

      if(l1TC.tmRpcBit[tp]==1){
        h1_quality1->Fill(l1TC.tmCode[tp]);
      }
      if(l1TC.tmRpcBit[tp]==2 && l1TC.tmSt[tp]==1){
        h1_quality2->Fill(l1TC.tmCode[tp]);
      }
      ...
      //if(l1TC.phSize > l1TC.tmSize)
      //print_event(&l1TC, &evtc);
      //hrpc_bx->Scale(0.01);
  ...
  gStyle->SetOptTitle(0);
   ...
   TLegend *leg;
   leg = new TLegend(0.62,0.62,0.95,0.82);
   leg->AddEntry(h1_bx,"TM Output", "l");
   ...
   c2d_rpcbx->SaveAs("Plots/twinmux_rpc_inbx_run"+RUN+LABEL+".png");
    ...
    TCanvas *c18 = canvas_data_mc_2d(h_nin_nout, "#DT Input", "#TM Output", 0,60, RUN, false); c18->SetLogz();
...
void print_event(L1TwinMuxTree* l1TC, Event* evtc){
  cout<<"Run = "<<evtc->run<<"\t Event = "<<evtc->event<<"\t Lumi = "<<evtc->lumi<<endl;
     ...
     cout<<"Output: PhiData"<<endl;
     cout<<"bx wh  ts2  st  sect phi_b phi code"<<endl;
     for(int tp=0; tp<l1TC->tmSize; tp++){ 
        if(l1TC->tmCode[tp]==7) continue;
         cout<<l1TC->tmBx[tp]<<"\t"<<l1TC->tmWh[tp]<<"\t"<<l1TC->tmTs2Tag[tp]<<"\t"<<l1TC->tmSt[tp]<<"\t"<<l1TC->tmSe[tp]<<"    "<<l1TC->tmBandAng[tp]<<"    "<<l1TC->tmAng[tp]<<"    "<<l1TC->tmCode[tp]<<endl;
      }
